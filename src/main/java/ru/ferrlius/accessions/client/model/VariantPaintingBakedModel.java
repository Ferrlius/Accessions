package ru.ferrlius.accessions.client.model;
...
public class VariantPaintingBakedModel extends BakedModelWrapper<BakedModel> {
    ...
    private static final class VariantOverrides extends ItemOverrides {
        ...
        public BakedModel resolve(
            ...
            ResourceLocation originalId = readOriginalVariantId(stack);
            if (originalId != null) {
                BakedModel originalModel = variantModels.get(originalId);
        ...
        @Nullable
        private static ResourceLocation readOriginalVariantId(ItemStack stack) {
            CustomData data = stack.getOrDefault(DataComponents.ENTITY_DATA, CustomData.EMPTY);
            if (data.isEmpty()) {
                return null;
            }
            ...
            return ResourceLocation.tryParse(tag.getString(PaintingStackHelper.ORIGINAL_VARIANT_TAG));
        }
    }
}
